...
> 生成日期：2026-07-23
> 测试套件：80 个用例（53 单元 `tests/test_core.py` + 27 mock pipeline `tests/mock_regression/`），**全部通过**
> 总体覆盖率：**55%**（6340 语句 / 2884 未覆盖）
...
| 维度 | 结论 |
|------|------|
| 单元测试覆盖度 | 总 **55%**；**业务编排层（pipeline）已 58%–80% 覆盖**，模型层 98% |
| 能否放进 GitHub Actions | ✅ **非常适合**——测试在 API 边界处全 mock，无网络、无需 API Key，素材已入库 |
| 当前最大缺口 | 外部 API 客户端（被有意 mock）、`server.py` HTTP 层、`artifacts.py` 级联删除、`config.py` 环境解析 |
| 立即可补的高价值项 | `artifacts.py` / `error_collector.py` / `config.py` 纯逻辑单测（易写、易暴露 bug） |
...
4. **本地一键评估**：
   ...
   ```
5. **CI 评估**：见 `.github/workflows/test.yml`，用 `pytest --cov` 并上传 `htmlcov/` 与 `coverage.xml` 作为 artifact。

> 覆盖率只是一个信号，不是目标本身。本项目约 1/3 代码是「外部 API 客户端 + FastAPI HTTP 层」，这部分本就不该用单元测试覆盖（应走集成测试），所以 55% 是合理水位，不必恐慌。
...
| `core/pipelines/__init__.py` | 72% | 基类/调度 |
...
| `core/pipelines/creative_video.py` | 60% | 创意管线（最大单文件 828 行，分支多） |
| `core/pipelines/multi_scene.py` | 58% | 多场景基类 |
| `core/screenwriter.py` | 51% | 编剧/分镜（大量 LLM 分支，本就被 mock） |
| `core/task_manager.py` | 51% | 任务生命周期 |
| `core/audio/subtitle.py` | 49% | 字幕生成（部分分支靠真实渲染触发） |
| `core/audio/tts.py` | 41% | TTS 引擎（EdgeTTS 被 mock） |
| `core/config.py` | 42% | 配置/环境变量解析 |
...
| `core/artifacts.py` | 19% | **纯逻辑**（级联删除） | ⭐ 高优先级补测 |
| `core/audio/voices.py` | 20% | 音色目录（含网络拉取） | 解析/缓存逻辑可 mock 单测 |
| `core/compositor/processor.py` | 25% | 后处理 | 可补 |
| `server.py` | 19% | FastAPI HTTP 层（888 行） | 用 `fastapi.testclient` 做路由单测 |
...
2. **`server.py` 19%（888 行，716 未覆盖）—— 最大单块缺口**
   FastAPI 路由层：鉴权、参数校验、任务 CRUD、文件下载等。当前没有任何 HTTP 层测试。可用 `fastapi.testclient.TestClient` + mock API key 低成本补上，能直接锁住「接口契约」。

3. **`artifacts.py` 19%（216 行）—— 最该补的纯逻辑**
   产物级联删除、磁盘清理。纯逻辑、无外部依赖，**单测极易写且能抓住真实 bug**（误删风险）。

4. **`config.py` 42% / `error_collector.py` 19% / `voices.py` 20%**
...
| 优先级 | 目标 | 预期收益 | 成本 |
|--------|------|----------|------|
| P0 | `artifacts.py` 级联删除/清理单测 | 防误删、覆盖率 +3% | 低 |
...
> 不做这轮补测，总覆盖率也会停在 ~55%；按上表补完 P0+P1，预计可到 **70%+**，且补齐的是「最容易出生产事故」的逻辑。
...
4. **`test_core.py` 纯单元**：模型/字幕/配置/拼接器，无任何外部依赖。

### 已落地的 `test.yml`

- 触发：`push` 到 `main`/`master`、所有 `pull_request`、`workflow_dispatch`
- 步骤：Checkout → 设 Python 3.12（带 pip 缓存）→ 装 ffmpeg → 装依赖 → `pytest --cov` → 上传 `htmlcov/` + `coverage.xml` artifact
- **防回归门禁**：`--cov-fail-under=50`，覆盖率跌破基线即小红叉
- 可选：把 `python-version` 改成矩阵可验证 3.11 / 3.12 多版本

> ⚠️ 一个 CI 注意点：字幕渲染依赖 CJK 字体 `resource/fonts/STHeitiMedium.ttc`（已入库）。
> 本地通过；ubuntu-latest 上该 `.ttc` 随仓库 checkout 可用，但需确认 moviepy 在 Linux 下能加载。
> 若 CI 出现「中文方块/字体缺失」，在 workflow 里补一步安装中文字体（`fonts-noto-cjk`）即可。

---

## 七、下一步

1. 把 `test.yml` + `pytest.ini` + `requirements-dev.txt` 提交，CI 即生效。
2. 按第五节 P0 补 `artifacts.py` 与 `server.py` 单测，把 `--cov-fail-under` 逐步提到 70。
