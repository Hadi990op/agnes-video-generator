...
from core.config import get_api_key, mark_key_429
...
class AgnesVideoAPI:
    ...
    def __init__(
        ...
        self._refresh_key_callback = None  # 多 key 轮转：外部注入的 key 刷新回调
        self.headers = {
            "Authorization": f"Bearer {api_key}",
            "Content-Type": "application/json",
        }

    def set_refresh_key_callback(self, callback):
        """注入 key 刷新回调（多 key 轮转时由 pipeline 设置）。

        回调签名: () -> str  （返回新的 api_key，或空字符串表示无法刷新）
        """
        self._refresh_key_callback = callback

    def _refresh_key(self) -> bool:
        """尝试刷新 API key（多 key 轮转）。返回 True 如果 key 已更新。"""
        if not self._refresh_key_callback:
            return False
        new_key = self._refresh_key_callback()
        if new_key and new_key != self.api_key:
            self.api_key = new_key
            self.headers["Authorization"] = f"Bearer {new_key}"
            logger.info(f"[AgnesVideo] Rotated to new API key: {new_key[:8]}...")
            return True
        return False
                ...
                if resp.status_code == 429:
                    ...
                    if self._refresh_key():
                        logger.info(
                            ...
                            f"retrying {mode_desc} immediately..."
                        )
                        collect_error(
                            "video", "submit_video",
                            prompt=payload.get("prompt", ""),
                            error_type="RateLimit429_KeyRotated",
                            error_message=f"HTTP 429: rotated to new key, retrying",
                            status_code=429,
                            response_body=resp.text,
                        ...
                        continue
                    # 无法刷新（单 key 或所有 key 都在冷却） → 正常等待重试
